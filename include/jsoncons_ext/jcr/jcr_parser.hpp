...
#include <jsoncons_ext/jcr/primitive_type_rules.hpp>
...
class basic_jcr_parser : private serializing_context
{
    ...
    basic_null_jcr_input_handler<CharT> default_input_handler_;
...
public:
    ...
    basic_jcr_parser(parse_error_handler& err_handler)
       : handler_(default_input_handler_),
    ...
    basic_jcr_parser(const CharT* input, size_t length, basic_jcr_input_handler<CharT>& handler)
       : handler_(handler),
         err_handler_(default_err_handler_),
         cp_(0),
         cp2_(0),
        ...
        while ((p_ < local_end_input) && (state_ != parse_state::done))
        {
            ...
            case parse_state::t: 
                ...
                case 'r':
                    ...
                    break;
                default:
                    err_handler_.error(jcr_parser_errc::invalid_value, *this);
    ...
    void parse_rule(std::error_code& ec)
    {
        const CharT* q = p_;
        while (q < end_input_ && !(*q == ' ' || *q == '\t' || *q == ',' || *q == '}' || *q == ']'))
        {
            ++q;
        }
        size_t len = q - p_;
        auto it = rule_dictionary.find(string_type(p_,q-p_));  
        if (it != rule_dictionary.end())
        {
            handler_.rule(it->second,*this);
            p_ += (q-p_);
            if (parent() == parse_state::root)
            {
                handler_.end_json();
        ...
        {
            err_handler_.error(jcr_parser_errc::illegal_escaped_character, *this);
...
minus_sign:
        ...
        switch (*p_)
        {
            ...
            default:
                err_handler_.error(jcr_parser_errc::expected_value, *this);
                ec = jcr_parser_errc::expected_value;
                return;
        }
integer_range:
...
negative_zero:
        ...
        switch (*p_)
        {
            ...
            case '0': case '1':case '2':case '3':case '4':case '5':case '6':case '7':case '8': case '9':
                err_handler_.error(jcr_parser_errc::leading_zero, *this);
                ec = jcr_parser_errc::leading_zero;
                state_ = parse_state::negative_zero;
                return;
            default:
                err_handler_.error(jcr_parser_errc::invalid_number, *this);
...
escape_expect_surrogate_pair1:
        ...
        }
        {
            ...
            case '\\': 
                ...
                goto escape_expect_surrogate_pair2;
            default:
                err_handler_.error(jcr_parser_errc::expected_codepoint_surrogate_pair, *this);
...
private:
    ...
    void end_fraction_value(const char* s, size_t length, std::error_code& ec)
    {
        try
        {
            ...
            {
                handler_.rule(std::make_shared<specific_double_rule>(d, static_cast<uint8_t>(std::numeric_limits<double>::max_digits10)), *this);
            }
            else
            {
                handler_.rule(std::make_shared<specific_double_rule>(d, static_cast<uint8_t>(precision_)), *this);
            }
        }
        catch (...)
        {
